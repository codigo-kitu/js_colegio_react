...
import {HeaderComp,HeaderCompBase} from '../../components/HeaderComp';
import {FooterComp} from '../../components/FooterComp';
import {AlertComp} from '../../components/AlertComp';

import {BuscarMateriaComp} from '../../components/estructura/materia/BuscarMateriaComp';
import {TablaDatosMateriaComp} from '../../components/estructura/materia/TablaDatosMateriaComp';
import {FormularioMateriaComp} from '../../components/estructura/materia/FormularioMateriaComp';
...
function MateriaView(): JSX.Element {	
	...
	const buscarmateriaComp1:any = useRef();
	const formulariomateriaComp1:any = useRef();
	...
	const getTodosDatos = () => {
		...
		buscarmateriaComp1.current.getTodosDatos();
		headerComp1.current.ocultarLoader();
	};
	
	const anteriores = () => {
		headerComp1.current.mostrarLoader();
		buscarmateriaComp1.current.anteriores();
		headerComp1.current.ocultarLoader();
	};
	
	const siguientes = () => {
		headerComp1.current.mostrarLoader();
		buscarmateriaComp1.current.siguientes();
		headerComp1.current.ocultarLoader();
	};
	...
	const nuevoPreparar = () => {
		formulariomateriaComp1.current.nuevoPreparar();
	};
	
	const handleAction_ClickTableRow = (materia1: Materia) => {
		formulariomateriaComp1.current.handleAction_ClickTableRow(materia1);
	};
	
	const handleAction_NuevoDatos = () => { //data_json
		buscarmateriaComp1.current.getTodosDatos();
		formulariomateriaComp1.current.cerrarModalFormGeneral();
	...
	<div id="divViewGlobalmateria">
		...
		<BuscarMateriaComp ref={buscarmateriaComp1}
		...
		<FormularioMateriaComp ref={formulariomateriaComp1}
					module={module} controller={controller}
					materia={materia}
					materias={materias}
